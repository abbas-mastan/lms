...
if (!function_exists('getCurrency')) {
    ...
    function getCurrency($short_code)
    {
         ...
         $ip = '162.159.24.227'; //request()->ip(); 
         $currentUserInfo = Location::get($ip);
         $countriesCurrency = $countries->currencies()->toArray()[$short_code];
         // dd($countries->currencies()->toArray()["CAD"]);
        // dd($countries->where('name.common', 'United States')->first()->currencies->toArray());
        // $currencies = config('currencies');
        return [
            'short_code'=> $short_code,
            'symbol'=> $countriesCurrency['units']['major']['symbol'] ??
            $countriesCurrency['iso']['code'],
            'name'=> $countriesCurrency['name'],
